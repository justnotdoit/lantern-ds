...
>(({ className, ...props }, ref) => {
  return (
    <Separator
      ...
      className={cn("mx-3 w-auto self-stretch bg-sidebar-border", className)}
      {...props}
    />
  );
});
  ...
  ({ className, ...props }, ref) => {
    return (
      <div
        ...
        className={cn(
          // px-2.5: вместе с px-3 кнопки даёт колонку иконок на x=22px —
          // ровно центр 60px-рейла, поэтому при сворачивании иконки не двигаются
          "flex min-h-0 flex-1 flex-col gap-3 overflow-auto px-2.5 pb-3 group-data-[collapsible=icon]:overflow-hidden",
          className,
        )}
        {...props}
      />
