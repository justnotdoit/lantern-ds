...
// --- Кастомные raw-шкалы (отличия от палитры Tailwind) -----------------------
// Например, violet — бренд-шкала Lantern. Классы вида bg-violet-500 у
// потребителей должны рисовать цвета Lantern, поэтому такие шкалы
// переопределяются в tailwind-конфиге (нашем и через theme-item).

const ignoreDiff = new Set(mapping.rawColorDiffIgnore?.ignore ?? []);
const customScales = {};
const rawPalette = {};
for (const [key, value] of Object.entries(rawColors)) {
  const [scale, shade] = key.split("/");
  if (!shade) continue; // white/black — без шкалы
  const lantern = String(value).toLowerCase();
  (rawPalette[scale] ??= {})[shade] = lantern;
  if (ignoreDiff.has(key)) continue;
  const tw = twColors[scale]?.[shade];
  if (tw && tw.toLowerCase() !== lantern) {
    (customScales[scale] ??= {})[shade] = lantern;
  }
}
// Шкала переопределяется целиком, чтобы не получить «полу-Lantern» микс
for (const scale of Object.keys(customScales)) {
  customScales[scale] = rawPalette[scale];
}

writeFileSync(
  path.join(rootDir, "tokens/custom-scales.js"),
...
export default ${JSON.stringify(customScales, null, 2)};
`,
);

writeFileSync(
  path.join(rootDir, "stories/foundations/raw-colors.generated.json"),
  `${JSON.stringify({ palette: rawPalette, customScales: Object.keys(customScales) }, null, 2)}\n`,
);

// --- registry.json (cssVars + tailwind у theme-lantern) -----------------------
...
if (Object.keys(customScales).length > 0) {
  themeItem.tailwind = {
    config: { theme: { extend: { colors: customScales } } },
  };
} else {
  delete themeItem.tailwind;
}
...
console.log(
  `✔ кастомные raw-шкалы (переопределены в tailwind): ${Object.keys(customScales).join(", ") || "нет"}`,
);
